...
class WeatherApiClientTest {
    ...
    fun `parseGeocode valid response`() {
        ...
        val result = WeatherApiClient.parseGeocode(body)
    ...
    fun `parseGeocode missing results array`() {
        assertNull(WeatherApiClient.parseGeocode("""{"generationtime_ms":0.5}"""))
    }

    @Test
    fun `parseGeocode empty results array`() {
        assertNull(WeatherApiClient.parseGeocode("""{"results":[]}"""))
    }
    ...
    fun `parseGeocodeSearch valid response`() {
        ...
        val results = WeatherApiClient.parseGeocodeSearch(body)
    ...
    fun `parseGeocodeSearch missing results`() {
        assertEquals(emptyList<GeocodingResult>(), WeatherApiClient.parseGeocodeSearch("""{}"""))
    }

    @Test
    fun `parseGeocodeSearch empty results`() {
        assertEquals(emptyList<GeocodingResult>(), WeatherApiClient.parseGeocodeSearch("""{"results":[]}"""))
    }
    ...
    fun `parseWeather valid response daytime`() {
        ...
        val result = WeatherApiClient.parseWeather(body)
        assertNotNull(result)
    ...
    fun `parseWeather missing current_weather returns null`() {
        assertNull(WeatherApiClient.parseWeather("""{"hourly":{}}"""))
    }

    @Test
    fun `parseWeather missing fields returns null`() {
        assertNull(WeatherApiClient.parseWeather("""{"current_weather":{"temperature":10}}"""))
    }

    @Test
    fun `parseWeather invalid JSON returns null`() {
        assertNull(WeatherApiClient.parseWeather("not json"))
    }
}
